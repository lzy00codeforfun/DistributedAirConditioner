...
speed = ['低','中','高']
rtypes = {'day':'日','week':'周','month':'月','year':'年'}
...
def printInvoice(filename, result):
	workbook = xlwt.Workbook()
	...
	worksheet.write_merge(1, 2, 1, 4, "{}-账单".format(result['room_id']), topicStyle)
...
def printReport(filename, rtype, result, begin_time=0, end_time=0):
	workbook = xlwt.Workbook()
	...
	if rtype == 'day':
		worksheet.write_merge(1, 2, 1, 8, "{}报表({}~{})".format(rtypes[rtype],begin_time,end_time), topicStyle)
	else:
		worksheet.write_merge(1,2,1,8,"{}报表({}~{})".format(rtypes[rtype],begin_time,end_time), topicStyle)
